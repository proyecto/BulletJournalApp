...
export const JournalProvider = ({ children }) => {
  ...
  const reorderLists = async (newOrder) => {
    ...
    setLists(newOrder);
    try {
      // Persistimos el cambio de orden en la BD en una sola transacción
      const updates = newOrder.map((list, index) => ({ id: list.id, newIndex: index }));
      await ListRepository.updateListsOrderTransaction(updates);
    } catch (e) {
  ...
  const reorderEntries = async (reorderedSubset) => {
    ...
    try {
      const updates = reorderedWithIndexes.map((entry, index) => ({ id: entry.id, newIndex: index }));
      await EntryRepository.updateEntriesOrderTransaction(updates);
    } catch (e) {
